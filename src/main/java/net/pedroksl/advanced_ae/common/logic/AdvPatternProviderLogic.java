...
public class AdvPatternProviderLogic implements InternalInventoryHost, ICraftingProvider {
    ...
    public AdvPatternProviderLogic(
            IManagedGridNode mainNode, AdvPatternProviderLogicHost host, int patternInventorySize) {
    ...
    public void updatePatterns() {
        ...
        for (var stack : this.patternInventory) {
            var details = PatternDetailsHelper.decodePattern(
                    stack, this.host.getBlockEntity().getLevel());
    ...
    private boolean doWork() {
        ...
        return returnInv.injectIntoNetwork(
                        mainNode.getGrid().getStorageService().getInventory(),
                        actionSource,
        ...
        if (player != null && input.contains(NBT_MEMORY_CARD_PATTERNS) && !player.level().isClientSide) {
            ...
            for (int i = 0; i < desiredPatterns.size(); i++) {
                ...
                var pattern = PatternDetailsHelper.decodePattern(
                        desiredPatterns.getStackInSlot(i), host.getBlockEntity().getLevel(), true);
                if (pattern == null) {
    ...
    public long getSortValue() {
        ...
        return te.getBlockPos().getZ() << 24
                ^ te.getBlockPos().getX() << 8
                ^ te.getBlockPos().getY();
    }
