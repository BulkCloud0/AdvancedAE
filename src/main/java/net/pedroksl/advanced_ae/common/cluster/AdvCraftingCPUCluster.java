...
public class AdvCraftingCPUCluster implements IAECluster {
    ...
    @Override public boolean isDestroyed() { return this.isDestroyed; }
    @Override public BlockPos getBoundsMin() { return this.boundsMin; }
    @Override public BlockPos getBoundsMax() { return this.boundsMax; }
    ...
    @Override public Iterator<AdvCraftingBlockEntity> getTiles() { return this.blockEntities.iterator(); }
    public Iterator<AdvCraftingBlockEntity> getBlockEntities() { return getTiles(); }
    public int numBlockEntities() { return this.blockEntities.size(); }

    void addBlockEntity(final AdvCraftingBlockEntity tile) {
        if (this.machineSrc == null || tile.isCoreBlock()) this.machineSrc = new MachineSource(tile);
        tile.setCoreBlock(false);
        tile.saveChanges();
        this.blockEntities.add(0, tile);
        if (tile.getStorageBytes() > 0) this.storage += tile.getStorageBytes();
        if (tile.getStorageMultiplier() > 0) this.storageMultiplier += tile.getStorageMultiplier();
        if (tile.getAcceleratorThreads() > 0) {
            if (tile.getAcceleratorThreads() > 16) throw new IllegalArgumentException("Co-processor threads may not exceed 16 per single unit block.");
            this.accelerator += tile.getAcceleratorThreads();
        }
        if (tile.getAccelerationMultiplier() > 0) this.acceleratorMultiplier += tile.getAccelerationMultiplier();
    ...
    public void recalculateRemainingStorage() {
        ...
        long usedStorage = 0;
        for (final AdvCraftingCPU cpu : this.activeCpus.values()) usedStorage += cpu.getAvailableStorage();
    ...
    public void markDirty() {
        ...
        if (core != null) core.saveChanges();
    }

    public IActionSource getSrc() { return Objects.requireNonNull(this.machineSrc, "Crafting cluster is not initialized"); }

    @Nullable
    private AdvCraftingBlockEntity getCore() {
        if (this.machineSrc == null || !this.machineSrc.machine().isPresent()) return null;
        return (AdvCraftingBlockEntity) this.machineSrc.machine().get();
    }

    @Nullable public IGrid getGrid() { final IGridNode node = getNode(); return node != null ? node.getGrid() : null; }

    public void cancelJobs() {
        final List<UUID> ids = new ArrayList<>(this.activeCpus.keySet());
        for (final UUID id : ids) killCpu(id, false);
        updateGridForChangedCpu(this);
    }

    public void cancelJob(final UUID uniqueId) { if (this.activeCpus.containsKey(uniqueId)) killCpu(uniqueId, true); }

    @Nullable
    public ICraftingLink submitJob(final IGrid grid, final ICraftingJob job, final IActionSource src,
            @Nullable final ICraftingRequester requestingMachine) {
        if (!isActive() || job == null || job.isSimulation() || getAvailableStorage() < job.getByteTotal()) return null;
    ...
    public List<AdvCraftingCPU> getActiveCPUs() {
        ...
        for (final Map.Entry<UUID, AdvCraftingCPU> entry : this.activeCpus.entrySet()) {
            ...
            else killList.add(entry.getKey());
        }
        for (final UUID cpuId : killList) this.activeCpus.remove(cpuId);
        if (!killList.isEmpty()) recalculateRemainingStorage();
        return list;
    }
    ...
    @Nullable public ITextComponent getName() { return this.myName; }
    @Nullable public IGridNode getNode() { final AdvCraftingBlockEntity core = getCore(); return core != null ? core.getActionableNode() : null; }
    public boolean isActive() { final IGridNode node = getNode(); return node != null && node.isActive(); }
    ...
    public void readFromNBT(final CompoundNBT data) {
        ...
        for (int i = 0; i < cpuList.size(); i++) {
            ...
            UUID id;
            try { id = UUID.fromString(pair.getString("key")); }
            catch (IllegalArgumentException ignored) { id = UUID.randomUUID(); }
    ...
    public void updateName() {
        ...
        for (final AdvCraftingBlockEntity tile : this.blockEntities) {
            if (tile.hasCustomInventoryName()) {
                this.myName = tile.getCustomInventoryName();
                break;
            }
        }
    }

    @Nullable public World getLevel() { final AdvCraftingBlockEntity core = this.getCore(); return core != null ? core.getLevel() : null; }
    public void breakCluster() { final AdvCraftingBlockEntity core = this.getCore(); if (core != null) core.breakCluster(); }
    public AdvCpuSelectionMode getSelectionMode() { return this.selectionMode; }
    public void setSelectionMode(final AdvCpuSelectionMode selectionMode) { this.selectionMode = selectionMode == null ? AdvCpuSelectionMode.ANY : selectionMode; markDirty(); }

    public boolean canBeAutoSelectedFor(final IActionSource source) {
        switch (getSelectionMode()) {
            case PLAYER_ONLY: return source.player().isPresent();
            case MACHINE_ONLY: return !source.player().isPresent();
            case ANY:
            default: return true;
        }
    }
