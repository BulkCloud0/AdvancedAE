...
public class UpgradeCards {
    public static boolean walkSpeed(Level level, Player player, ItemStack stack) {
        ...
        boolean canFly = chest.getItem() instanceof QuantumArmorBase
                && ((QuantumArmorBase) chest.getItem()).isUpgradeEnabledAndPowered(chest, UpgradeType.FLIGHT);
        ...
        if (!player.isSprinting() && (canFly || isNotFlying) && !player.isInWaterOrBubble()) {
            ...
            if (stack.getItem() instanceof QuantumLeggings
                    && ((QuantumLeggings) stack.getItem()).isUpgradeEnabledAndPowered(stack, upgrade)) {
    ...
    private static boolean processMovementSpeed(
        ...
        if (stack.getItem() instanceof QuantumLeggings) {
            ...
            if (canFly
                    ...
                    && chest != null
                    && chest.getItem() instanceof QuantumChestplate) {
        ...
        if (player.getFoodData().needsFood() && stack.getItem() instanceof QuantumHelmet) {
            ...
            if (helmet.isUpgradeEnabledAndPowered(stack, UpgradeType.AUTO_FEED) && helmet.getLinkedPosition(stack) != null) {
                ...
                if (grid != null) {
                    ...
                    List<GenericStack> filter = helmet.getFilter(stack, UpgradeType.AUTO_FEED);
                    for (GenericStack genStack : filter) {
                        if (storage.getInventory()
                            ...
                            FoodProperties foodProperties = foodStack.getFoodProperties(player);
                            if (foodProperties != null) {
                                storage.getInventory()
                                        .extract(genStack.what(), 1, Actionable.MODULATE, IActionSource.ofPlayer(player));
    ...
    public static boolean autoStock(Level level, Player player, ItemStack stack) {
        if (stack.getItem() instanceof QuantumHelmet) {
            QuantumHelmet helmet = (QuantumHelmet) stack.getItem();
            if (helmet.isUpgradeEnabledAndPowered(stack, UpgradeType.AUTO_STOCK) && helmet.getLinkedPosition(stack) != null) {
                ...
                if (grid != null) {
                    IStorageService storage = grid.getStorageService();
                    List<GenericStack> filter = helmet.getFilter(stack, UpgradeType.AUTO_STOCK);
                    boolean didSomething = false;
                    KeyCounter inventory = storage.getInventory().getAvailableStacks();
                    for (GenericStack genStack : filter) {
                        if (genStack.what() instanceof AEItemKey) {
                            AEItemKey itemKey = (AEItemKey) genStack.what();
    ...
    public static boolean magnet(Level level, Player player, ItemStack stack) {
        if (!player.isSpectator() && stack.getItem() instanceof QuantumHelmet) {
            ...
            if (helmet.isUpgradeEnabledAndPowered(stack, UpgradeType.MAGNET)) {
                ...
                List<GenericStack> filter = helmet.getFilter(stack, UpgradeType.MAGNET);
    ...
    public static boolean recharging(Level level, Player player, ItemStack stack) {
        if (stack.getItem() instanceof QuantumArmorBase) {
            ...
            if (armor.isUpgradeEnabledAndPowered(stack, UpgradeType.CHARGING) && armor.getLinkedPosition(stack) != null) {
    ...
    private static void rechargeItem(
        ...
        LazyOptional<IEnergyStorage> capOf = stack.getCapability(ForgeCapabilities.ENERGY);
