...
public interface IUpgradeableItem extends IGridLinkedItem {
    ...
    default List<UpgradeType> getPassiveUpgrades(ItemStack itemStack) {
        final List<UpgradeType> abilityList = new ArrayList<UpgradeType>();
    ...
    default boolean isUpgradePowered(@NotNull ItemStack stack, UpgradeType upgrade) {
        LazyOptional<IEnergyStorage> energyOp = stack.getCapability(ForgeCapabilities.ENERGY);
    ...
    default boolean toggleUpgrade(ItemStack stack, UpgradeType type, Player player) {
        if (hasUpgrade(stack, type)) {
            ...
            if (tag != null) {
                ...
                if (player != null) {
                    MutableComponent id = Component.translatable(type.item().asItem().getDescriptionId());
