...
public class BlockEntityRegistry {
    ...
    private static DeferredRegister<TileEntityType<?>> getRegister(String modId) {
        ...
        if (register == null) throw new IllegalStateException("Tile entity registry not initialized for mod " + modId);
        return register;
    }
    ...
    protected static <T extends AEBaseTileEntity> Supplier<TileEntityType<T>> create(
            String modId, String id, Class<T> entityClass, BlockEntityFactory<T> factory,
            LibBlockDefinition<? extends Block>... blockDefs) {
        if (blockDefs.length == 0) throw new IllegalArgumentException("At least one block is required for tile entity " + id);

        return getRegister(modId).register(id, () -> {
            Block[] blocks = Arrays.stream(blockDefs).map(LibBlockDefinition::block).toArray(Block[]::new);
            AtomicReference<TileEntityType<T>> typeRef = new AtomicReference<>();
            Supplier<T> tileFactory = () -> factory.create(typeRef.get(), BlockPos.ZERO, null);
            ...
            AEBaseTileEntity.registerTileItem(entityClass, new IStackSrc() {
                @Override public ItemStack stack(int amount) { return primaryBlock.stack(amount); }
                @Override public Item getItem() { return primaryBlock.asItem(); }
    ...
    public void register(IEventBus eventBus) { getRegister(modId).register(eventBus); }
